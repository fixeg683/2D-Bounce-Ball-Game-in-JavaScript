...
// Modify the keyDownHandler to toggle pause state when the "P" key is pressed
function keyDownHandler(e) {
    if (e.keyCode === 39) {
        rightPressed = true;
    } else if (e.keyCode === 37) {
        leftPressed = true;
    } else if (e.keyCode === 80) { // "P" key for pause
...
// Modify the draw function to respect the paused state
function draw() {
    if (!isPaused) {
        ...
        // Collision detections and paddle movements
        if (x + dx > canvas.width - ballRadius || x + dx < ballRadius) {
            dx = -dx;
        }
        if (y + dy < ballRadius) {
